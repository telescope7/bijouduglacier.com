...
# Hosts the markup is allowed to name in a src/href. These are link targets a
# visitor clicks, not resources the browser fetches, so they cost no request
# and no privacy. Anything else is a third-party dependency and fails the
# audit: the site promises zero of those.
ALLOWED_OUTBOUND_HOSTS = (
    "schema.org", "www.w3.org",
    "saasfeeholidays.com", "www.airbnb.com", "www.booking.com",
)
...
def main():
    ...
    # --- 10c. link walk: is every page reachable, and every link real? ---
    # Walks from website/index.html following the meta-refresh and every
    # <a href>, resolving directory links to index.html the way a web server
    # does, and checks the stylesheet resolves from every page it lands on.
    # Proves there are no broken internal links and no orphaned pages.
    ...
    while queue:
        ...
        for h in local:
            # fonts/fonts.css is generated by tools/fetch_fonts.py and has its
            # own dedicated check below, which gives an actionable message.
            # Reporting it seventeen times here would bury that.
            if h.endswith("fonts/fonts.css"):
                continue
        ...
        for h in parsed[cur].links:
            ...
            # Page links are now the canonical directory form, so resolve a
            # directory to its index.html exactly as nginx does with
            # `try_files $uri $uri/`. website/index.html still uses explicit
            # filenames and is handled by the isfile branch.
            if os.path.isdir(t):
                idx = os.path.join(t, "index.html")
                if not os.path.isfile(idx):
                    fail(rel(cur), "link %s points at a directory with no "
                                   "index.html" % h)
                else:
                    queue.append(idx)
    ...
    if unreached:
        fail("link walk", "unreachable by clicking from website/index.html: %s"
             % ", ".join(unreached))
    else:
        print("  OK    link walk reached all %d pages, every internal link "
              "resolves\n" % len(seen))

    # --- 10c-bis. internal links must match the canonical form -----------
    # Every page link should be the directory form. An index.html link is a
    # link to a URL our own canonical tag tells Google not to index, and nginx
    # 301s it, so it costs a redirect on every click too. website/index.html is
    # exempt: it is noindex and is the entry point for opening the build from
    # disk.
    root_index = os.path.join(WEB, "index.html")
    for path in pages:
        if os.path.samefile(path, root_index):
            continue
        for h in parsed[path].links:
            if h.startswith(("http", "#", "mailto:", "tel:")):
                continue
            if urlparse(h).path.endswith("index.html"):
                fail(rel(path), "internal link %s uses the non-canonical "
                                "index.html form" % h)

    # --- 10c-ter. no JavaScript, no third-party requests -----------------
    # Two standing promises about this site: it runs no scripts, and it talks
    # to no other server. Both are easy to break by accident and invisible
    # once broken, so they are checked rather than trusted.
    for path in pages:
        raw = open(path, encoding="utf-8").read()
        for m in re.finditer(r"<script([^>]*)>", raw):
            if "application/ld+json" not in m.group(1):
                fail(rel(path), "contains a <script> tag that is not JSON-LD")
        for attr in ("onclick", "onload", "onerror", "onmouseover", "onfocus"):
            if attr + "=" in raw.lower():
                fail(rel(path), "inline event handler %s= found" % attr)
        # Any absolute URL in a src/href that isn't our own origin and isn't a
        # booking destination the markup deliberately names.
        for m in re.finditer(r'(?:src|href)="(https?://[^"]+)"', raw):
            u = m.group(1)
            if u.startswith(ORIGIN):
                continue
            if any(d in u for d in ALLOWED_OUTBOUND_HOSTS):
                continue
            fail(rel(path), "third-party request to %s" % u)

    # --- 10c-quater. the self-hosted fonts must actually be there --------
    font_dir = os.path.join(WEB, "fonts")
    font_css = os.path.join(font_dir, "fonts.css")
    if not os.path.isfile(font_css):
        fail("fonts/", "the self-hosted webfonts have not been downloaded yet. "
                       "Run:  cd tools && python3 fetch_fonts.py   "
                       "(one-off; needs network. Until then every page falls "
                       "back to Georgia and Helvetica.)")
    else:
        css = open(font_css, encoding="utf-8").read()
        refs = re.findall(r"url\('([^']+\.woff2)'\)", css)
        if not refs:
            fail("fonts/fonts.css", "declares no woff2 faces")
        for f in set(refs):
            fp = os.path.join(font_dir, f)
            if not os.path.isfile(fp):
                fail("fonts/fonts.css", "references %s, which is not on disk" % f)
            elif os.path.getsize(fp) < 5000:
                # A real latin woff2 subset of either family is 15-40 KB. A
                # file this small is a stub or a truncated download: it passes
                # an existence check and renders as nothing.
                fail("fonts/%s" % f, "only %d bytes — not a real font. Re-run "
                                     "tools/fetch_fonts.py"
                                     % os.path.getsize(fp))
        # Anything in fonts/ that fonts.css does not reference is dead weight:
        # never requested by a browser, but rsynced to the server on every
        # deploy. This is exactly what a change of naming scheme leaves behind.
        on_disk = {f for f in os.listdir(font_dir) if f.endswith(".woff2")}
        orphans = sorted(on_disk - set(refs))
        if orphans:
            waste = sum(os.path.getsize(os.path.join(font_dir, f))
                        for f in orphans)
            fail("fonts/", "%d font file(s) not referenced by fonts.css, "
                           "%.0f KB of dead weight. Re-run tools/fetch_fonts.py, "
                           "which now deletes them. (%s%s)"
                           % (len(orphans), waste / 1024.0,
                              ", ".join(orphans[:3]),
                              ", ..." if len(orphans) > 3 else ""))

        # Every @font-face that shares a family/weight/style with another must
        # carry a unicode-range, or the later rule silently replaces the
        # earlier one for the whole character set and half the glyphs vanish.
        ...
        for b in blocks:
            ...
            if not (fam and wt):
                continue
            k = (fam.group(1), wt.group(1), st.group(1) if st else "normal")
            keyed.setdefault(k, []).append(b)
        for k, bs in keyed.items():
            if len(bs) > 1 and not all("unicode-range" in b for b in bs):
                fail("fonts/fonts.css",
                     "%s %s has %d faces but not all declare a unicode-range — "
                     "the last one wins and the others never load"
                     % (k[0], k[1], len(bs)))

        # The faces preloaded in every page head come from the manifest, so the
        # filenames are never written down in two places.
        mf = os.path.join(font_dir, "manifest.json")
        if not os.path.isfile(mf):
            fail("fonts/manifest.json", "missing — re-run tools/fetch_fonts.py")
        else:
            m = json.load(open(mf, encoding="utf-8"))
            for role in ("serif", "sans"):
                f = m.get(role)
                if not f:
                    fail("fonts/manifest.json", "no %r entry" % role)
                elif not os.path.isfile(os.path.join(font_dir, f)):
                    fail("fonts/", "%s is preloaded by every page but missing" % f)
                else:
                    for path in indexable:
                        if f not in open(path, encoding="utf-8").read():
                            fail(rel(path), "does not preload %s" % f)
                            break
    ...
    for path in indexable:
        ...
        if "fonts.googleapis.com" in raw or "fonts.gstatic.com" in raw:
            fail(rel(path), "still links to Google Fonts; the fonts are self-hosted now")
        if 'as="font"' not in raw:
            warn(rel(path), "no font preload — first paint will swap typeface")
